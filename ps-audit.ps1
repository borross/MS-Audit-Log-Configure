...
#Updated script https://github.com/packetzero/ps-audit-scripts
#This script must be run with admin rights

Write-Host ""
Write-Host "Copyright (c) [2023] [RU Pre-Sales KL(Unofficial)]"  -ForegroundColor Green 
Write-Host "__________         ___.   .__              .___      "  -ForegroundColor Red 
Write-Host "\______   \        \_ |__ |  | _____     __| _/____  "  -ForegroundColor Red 
Write-Host " |    |  _/  ______ | __ \|  | \__  \   / __ |/ __ \ "  -ForegroundColor Red 
Write-Host " |    |   \ /_____/ | \_\ \  |__/ __ \_/ /_/ \  ___/ "  -ForegroundColor Red 
Write-Host " |______  /         |___  /____(____  /\____ |\___  >"  -ForegroundColor Red 
Write-Host "        \/              \/          \/      \/    \/ "  -ForegroundColor Red 
Write-Host ""
Write-Host "Script for Local Log Policy Audit Management"  -ForegroundColor Green 
Write-Host ""
...
Function GetHelp
{
    ...
    Write-Host " -h  ->  Get This Help"  -ForegroundColor Green
    Write-Host " -l  ->  Load and Save in CSV (ActualSetting_ComputerName.csv) Actual Audit Settings"  -ForegroundColor Green
	Write-Host " -p AuditTemplate.csv  ->  Print Audit Settings from AuditTemplate.csv file"  -ForegroundColor Green
    Write-Host " -c AuditTemplate.csv  ->  Compare Audit Settings with Desired AuditTemplate.csv file"  -ForegroundColor Green
    Write-Host " -s AuditTemplate.csv  ->  Sync Audit Settings with Desired AuditTemplate.csv file"  -ForegroundColor Green
    Write-Host ""
}
...
class ZAuditSetting
{
    ...
    [string] ToString( ) {
		return "===," + $this.Category + "`n" + $this.IsSuccessEnabled + $this.IsFailureEnabled + "," + $this.SubCategory
...
#------------------------------------------------------------------------
Function exportCurrentToFile()
{
	outFile=".\ExportCurrent.csv"
    foreach ($desired in $desiredAuditSettings) {
        $key = $desired.GetKey()
        if ($actualAuditMap.ContainsKey($key)) {
            $actual = $actualAuditMap.$key
...
switch ( $args[0] )
{
    ...
    "-p"
    {
		# Print Desired Settings From CSV File
		$desiredAuditSettings = LoadSettingsFromCsv $policyCSV
    ...
    "-l"
    {
        # Load and Save in csv Actual Settings
		$settingFile = ".\ActualSetting_"+$env:computername+".csv"
        $actualAuditSettings = GetAuditSettings
        $actualAuditMap = @{}
        foreach ($obj in $actualAuditSettings) { $actualAuditMap[$obj.GetKey()] = $obj }
        
		Start-Transcript -Path $settingFile
		PrintAuditSettings $actualAuditSettings "Actual"
		Stop-Transcript
		
		#(Get-Content -path .\testlog.txt).replace('TrueTrue', 'SF') | Set-Content .\testlog.txt
		(Get-Content -path $settingFile | Select-Object -Skip 21 | Select-Object -SkipLast 5) | Foreach-Object {
		$_ -replace 'TrueTrue', 'SF' `
		   -replace 'TrueFalse', 'S' `
		   -replace 'FalseTrue', 'F' `
		   -replace 'FalseFalse', '-'
		} | Set-Content $settingFile
